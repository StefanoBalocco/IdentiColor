...
type IdentiColorFunction = ( hash: string, shapes?: number ) => string;

type IdentiColorBundle = {
	default: IdentiColorFunction;
};

type BundleFixture = {
	name: string;
	bundle: IdentiColorBundle;
};
...
const REFERENCE_HASH: string = '65dee7708ba413165fc2eaba3e991443d013f397b29e2c8acdced248c442e0ca';
const ALL_F_HASH: string = 'f'.repeat( 64 );
const SHAPE_COUNTS: readonly number[] = [ 3, 4, 5 ];
const INVALID_SHAPES: readonly number[] = [ 0, 1, 2, 6 ];
const LONG_HASH: string = '00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff';
const UNSUPPORTED_LENGTH_HASH: string = 'f'.repeat( 24 );
const ALLOCATION_FIXTURES: readonly AllocationFixture[] = [
...
const bundleFixtures: BundleFixture[] = [
	{
		name: 'original>',
		bundle: ( await import( `${ distPath }/IdentiColor.js` ) ) as IdentiColorBundle,
	},
	{
		name: 'minified>',
		bundle: ( await import( `${ distPath }/IdentiColor.min.js` ) ) as IdentiColorBundle,
	},
];
...
const GOLDEN_SVG: string = '<svg x="0" y="0" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><rect x="0" y="0" width="100" height="100" fill="#f7d9c4" /><rect x="0" y="0" width="100" height="100" fill="#c9e4de" transform="rotate(-25.5,109.94831240731133,50) translate(59.94831,0.00000)" /><rect x="0" y="0" width="100" height="100" fill="#f2c6de" transform="rotate(10.5,50.00000000000001,119.03485394458738) translate(0.00000,69.03485)" /><rect x="0" y="0" width="100" height="100" fill="#c6def1" transform="rotate(3.0,-23.788336849574307,50.00000000000001) translate(-73.78834,0.00000)" /></svg>';

const cL1: number = bundleFixtures.length;
for( let iL1: number = 0; iL1 < cL1; iL1++ ) {
	const fixture: BundleFixture = bundleFixtures[ iL1 ];
	const bundleName: string = fixture.name;
	const IdentiColor: IdentiColorFunction = fixture.bundle.default;

	test( bundleName + ' default call returns string and equals explicit four shapes', ( t: ExecutionContext ) => {
		const defaultSvg: string = IdentiColor( REFERENCE_HASH );
		const explicitSvg: string = IdentiColor( REFERENCE_HASH, 4 );
		t.is( typeof defaultSvg, 'string' );
		t.is( defaultSvg, explicitSvg );
	} );

	test( bundleName + ' identical inputs produce identical SVG', ( t: ExecutionContext ) => {
		const firstSvg: string = IdentiColor( REFERENCE_HASH, 4 );
		const secondSvg: string = IdentiColor( REFERENCE_HASH, 4 );
		t.is( firstSvg, secondSvg );
	} );

	test( bundleName + ' different full hashes produce different SVG', ( t: ExecutionContext ) => {
		t.not( IdentiColor( REFERENCE_HASH ), IdentiColor( ALL_F_HASH ) );
	} );

	test( bundleName + ' uppercase hash equals lowercase hash', ( t: ExecutionContext ) => {
		t.is( IdentiColor( REFERENCE_HASH.toUpperCase() ), IdentiColor( REFERENCE_HASH ) );
	} );

	test( bundleName + ' shapes three four five emit three four five rects', ( t: ExecutionContext ) => {
		const cL2: number = SHAPE_COUNTS.length;
		for( let iL2: number = 0; iL2 < cL2; iL2++ ) {
			const shapeCount: number = SHAPE_COUNTS[ iL2 ];
			const svg: string = IdentiColor( REFERENCE_HASH, shapeCount );
			t.is( svg.split( '<rect' ).length - 1, shapeCount );
		}
	} );

	test( bundleName + ' emits structurally valid SVG without NaN or undefined', ( t: ExecutionContext ) => {
		const svg: string = IdentiColor( REFERENCE_HASH );
	...
	test( bundleName + ' invalid shapes throw RangeError', ( t: ExecutionContext ) => {
		const cL2: number = INVALID_SHAPES.length;
		for( let iL2: number = 0; iL2 < cL2; iL2++ ) {
			const invalidShape: number = INVALID_SHAPES[ iL2 ];
			t.throws( () => IdentiColor( REFERENCE_HASH, invalidShape ), { instanceOf: RangeError, message: 'Shapes must be between 3 and 5, inclusive.' } );
		}
	} );

	test( bundleName + ' valid-length allocations emit valid SVG', ( t: ExecutionContext ) => {
		const cL2: number = ALLOCATION_FIXTURES.length;
		for( let iL2: number = 0; iL2 < cL2; iL2++ ) {
			const allocationFixture: AllocationFixture = ALLOCATION_FIXTURES[ iL2 ];
			const svg: string = IdentiColor( allocationFixture.hash, allocationFixture.shapes );
	...
	test( bundleName + ' examples match committed SVG', ( t: ExecutionContext ) => {
		const cL2: number = exampleFixtures.length;
		for( let iL2: number = 0; iL2 < cL2; iL2++ ) {
			const exampleFixture: ExampleFixture = exampleFixtures[ iL2 ];
			t.is( IdentiColor( exampleFixture.hash, exampleFixture.shapes ), exampleFixture.svg );
		}
	} );

	test( bundleName + ' unsupported hash length throws RangeError', ( t: ExecutionContext ) => {
		t.throws( () => IdentiColor( UNSUPPORTED_LENGTH_HASH ), { instanceOf: RangeError, message: /^Unsupported hash length\. Use / } );
	} );

	test( bundleName + ' golden SVG matches the reference hash', ( t: ExecutionContext ) => {
